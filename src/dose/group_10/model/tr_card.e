...
feature{ANY}
	...
	make(card_type:STRING;card_value:INTEGER)
		...
		if value>=4 and value<=7 then
			weight_truco:=value-4
		elseif value>=10 and value<=12 then
			weight_truco:=value-6
		elseif value>=1 and value<=3 then
			weight_truco:=value+6
		end
		if value = 7 and type.is_equal ( "golds" )then
			weight_truco:=10
		elseif value = 7 and type.is_equal ( "swords") then
			weight_truco:=11
		elseif value = 1 and type.is_equal ( "clubs" )then
			weight_truco:=12
		elseif value = 1 and type.is_equal ( "swords") then
			weight_truco:=13 end
